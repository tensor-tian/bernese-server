...
type Quad3 = Tuple<string, 3>;
type Quad4 = Tuple<string, 4>;
type Quads = Quad3 | Quad4 | (Quad3 | Quad4)[];

declare module "gizmo" {
  /**
  ...
  interface Path {
    ...
    both(path: Path | string, ...tags: string[]): Path;
...
but do not follow the path, merely filter the possible paths. Usually useful for starting with all nodes, or limiting to a subset depending on some predicate/value pair.
  ...
  */
     ...
     */
    in: (
      predicatePath?: Path | string | string[] | null | undefined,
      ...tags: (string[] | string)[]
    ) => Path;
     ...
     */
    labelContext(
      labelPath: Path | null | undefined | string[] | string,
      ...tags: string[]
    ): Path;
